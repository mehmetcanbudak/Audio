#!/usr/bin/env bash
export board="teensy:avr:teensy4b:usb=serial,speed=396,opt=o2std,keys=en-us"
# define colors
...
for example in "${examples[@]}"; do
  echo -n $example:
  $HOME/arduino_ide/arduino-$ARDUINO_IDE_VERSION/arduino --verify --board "$board" $example &> error.txt
